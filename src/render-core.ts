...
import type { ThemeConfig } from './theme-schema.js';
...
export function createRenderer(getTheme: (name: string | undefined | null) => ThemeConfig) {
  ...
  function render(table: TableData, opts: RenderOptions = {}): string {
    const theme = getTheme(opts.theme);
